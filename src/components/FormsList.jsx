...
// Share Modal Component (inline)
function ShareModal({ isOpen, onClose, link }) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
      <div className="bg-white rounded-lg shadow-lg p-6 w-96">
        <h2 className="text-lg font-bold mb-4 text-gray-800">Share Form</h2>
        <input
          type="text"
          value={link}
          readOnly
          className="w-full border px-3 py-2 rounded mb-4 text-gray-700"
        />
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
      <div className="bg-white rounded-lg shadow-lg p-6 w-96">
        ...
        <p className="text-gray-600 mb-6">
          ...
          <span className="font-semibold">{formTitle}</span>? This will also
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/forms/${selectedForm._id}`,
        { method: "DELETE" }
      );
